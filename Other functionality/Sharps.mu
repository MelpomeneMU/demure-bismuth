...
@@ +channel/create Noms=A channel for +noms!
@@ +channel/staff Noms
@@ The channel +noms emit to.
...
&tr.wipe-weekly-nom-stats [v(d.sb)]=@break gettimer(%vS, global-noms); @eval settimer(%vS, global-noms, sub(mul(xget(%vS, d.noms-days), 60, 60, 24), 600)); @dolist lattr(%vS/global-noms-*) LAST-RESET={ @mvattr %vS=##, edit(##, GLOBAL-, PAST-, LAST-, PAST-); }; @set %vS=last-reset:[prettytime()]; @dolist search(EPLAYER=hasattr(##, _past-noms))={ @wipe ##/_past-noms; };
...
@@ %0: attribute
&f.get-badge-meaning [v(d.sb)]=trim(rest(rest(rest(rest(rest(default(strcat(%vS, /, %0), Date Time AMPM Creator: None: No meaning given.)))), :), :))

&f.get-badge-name [v(d.sb)]=trim(first(rest(rest(rest(rest(default(strcat(%vS, /, %0), Date Time AMPM Creator: None: No meaning given.)))), :), :))

&f.get-badge-creator [v(d.sb)]=trim(first(rest(rest(rest(default(strcat(%vS, /, %0), Date Time AMPM Creator: None: No meaning given.)))), :))

&f.get-badge-hidden [v(d.sb)]=t(member(xget(%0, _hidden-badges), %1, |))

&f.get-badge-creation-date [v(d.sb)]=extract(default(strcat(%vS, /, %0), Date Time AMPM Creator: None: No meaning given.), 1, 3)

&f.get-badge-status [v(d.sb)]=default(strcat(%vS, /, edit(%0, BADGE-, status-)), Active)

&f.get-badge-players [v(d.sb)]=default(strcat(%vS, /, edit(%0, BADGE-, players-)), if(t(setr(S, xget(%vS, edit(%0, BADGE-, sharpcount-)))), words(search(EPLAYER=gt(default(##/_total-sharps, 0), %qS))) players, None))

@@ %0: name
&f.find-badge-by-name [v(d.sb)]=if(t(%0), first(iter(lattr(%vS/badge-*), if(strmatch(strip(ulocal(f.get-badge-name, itext(0))), strip(%0)*), itext(0)))))
...
&layout.noms-global [v(d.sb)]=strcat(header(Global +noms stats, %0), if(isstaff(%0), ulocal(layout.noms-present, %0)), ulocal(layout.noms-past, %0), %r, footer(if(t(gettimer(%vS, global-noms)), cat(Reset in, getremainingtime(%vS, global-noms).), Reset once the @daily hits), %0))

&layout.noms-present [v(d.sb)]=strcat(%r,if(t(setr(L, xget(%vS, last-reset))), setq(L, %bsince %qL)), divider(+noms given%qL, %0), %r, multicol(if(t(setr(R, xget(%vS, global-noms-given))), strcat(Name|noms|Name|noms|Name|noms|, iter(%qR, strcat(setr(N, ulocal(f.get-name, first(itext(0), -), %0)), |, rest(itext(0), -)),, |)), None yet.), * 4 * 4 * 4, t(%qR), |, %0), %r, divider(+noms received%qL, %0), %r, multicol(if(t(setr(R, xget(%vS, global-noms-received))), strcat(Name|noms|Name|noms|Name|noms|, iter(%qR, strcat(setr(N, ulocal(f.get-name, first(itext(0), -), %0)), |, rest(itext(0), -)),, |)), None yet.), * 4 * 4 * 4, t(%qR), |, %0))

&layout.noms-past [v(d.sb)]=strcat(setq(C, xget(%vS, last-reset)), setq(L, xget(%vS, past-reset)), setq(L, case(strcat(t(%qL), t(%qC)), 11, %bbetween %qL and %qC, 01, %bbefore %qC, 10, %bafter %qL)), %r, divider(+noms given%qL, %0), %r, multicol(if(t(setr(R, xget(%vS, past-noms-given))), strcat(Name|noms|Name|noms|Name|noms|, iter(%qR, strcat(setr(N, ulocal(f.get-name, first(itext(0), -), %0)), |, rest(itext(0), -)),, |)), None yet.), * 4 * 4 * 4, t(%qR), |, %0), %r, divider(+noms received%qL, %0), %r, multicol(if(t(setr(R, xget(%vS, past-noms-received))), strcat(Name|noms|Name|noms|Name|noms|, iter(%qR, strcat(setr(N, ulocal(f.get-name, first(itext(0), -), %0)), |, rest(itext(0), -)),, |)), None yet.), * 4 * 4 * 4, t(%qR), |, %0))
...
&c.+nom/alias [v(d.sb)]=$+nom/*:@break switch(%0, s*, 1, a*, 1, q*, 1, 0); @break cand(isstaff(%#), strmatch(%0, )); @force %#=+noms/%0;

&c.+noms/alias [v(d.sb)]=$+noms/*:@break switch(%0, report, 1, report *, 1, global, 1, 0); @switch/first %0=r*, { @switch/first %0=r* me, { @force %#=+noms/report; }, { @force %#=+noms/report [rest(%0)]; }; }, g*, { @force %#=+noms/global; }, { @pemit %#=error(); };
...
&layout.badges [v(d.sb)]=strcat(header(All badges, %1), %r, edit(multicol(iter(lattr(%vS/badge-*), strcat(switch(ulocal(f.get-badge-status, itext(0)), R*, %ch%cxR%cn, _), |, ulocal(f.get-badge-name, itext(0))),, |), 1 * 1 * 1 *, 0, |, %1), _, %b), %r, footer(, %1))
...
&c.+badge/award [v(d.sb)]=$+badge/award *=*:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to manage badges.; }; @assert t(setr(P, ulocal(f.find-player, %0, %#)))={ @trigger me/tr.error=%#, Could not find a player named '%0'.; }; @assert t(setr(B, ulocal(f.find-badge-by-name, %1)))={ @trigger me/tr.error=%#, There is no badge named %1.; }; @eval setq(N, ulocal(f.get-badge-name, %qB)); @assert strmatch(ulocal(f.get-badge-status, %qB), Active)={ @trigger me/tr.error=%#, %qN is a retired badge - no one else may be awarded it.; }; @assert not(t(finditem(xget(%qP, _badges), %qN, |)))={ @trigger me/tr.error=%#, ulocal(f.get-name, %qP, %#) already has the badge '%qN'.; }; @set %qP=_badges:[squish(trim(strcat(xget(%qP, _badges), |, %qN), b, |), |)]; @set %vS=[edit(%qB, BADGE-, players-)]:[squish(trim(strcat(xget(%vS, edit(%qB, BADGE-, players-)), |, ulocal(f.get-name, %qP)), b, |), |)]; @trigger me/tr.success=%#, cat(You award, ulocal(f.get-name, %qP, %#), the %qN badge.); @trigger me/tr.log=%qP, _sharps-, %#, Awarded the %qN badge.; @trigger me/tr.message=%qP, cat(ulocal(f.get-name, %#, %qP) awards you the %qN badge! You can wear it with %ch+badge/wear %qN%cn. Here is your new badge's meaning:, ulocal(f.get-badge-meaning, %qB));
...
&c.+badge/remove [v(d.sb)]=$+badge/remove *=*:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to manage badges.; }; @assert t(setr(P, ulocal(f.find-player, %0, %#)))={ @trigger me/tr.error=%#, Could not find a player named '%0'.; }; @assert t(setr(B, ulocal(f.find-badge-by-name, %1)))={ @trigger me/tr.error=%#, There is no badge named %1.; }; @eval setq(N, ulocal(f.get-badge-name, %qB)); @assert t(finditem(xget(%qP, _badges), %qN, |))={ @trigger me/tr.error=%#, ulocal(f.get-name, %qP, %#) doesn't have the badge '%qN'.; }; @assert gettimer(%#, remove-%qP, %qN)={ @trigger me/tr.message=%#, You are about to remvoe the badge %qN from [ulocal(f.get-name, %qP, %#)]. Are you sure? If so%, type %ch+badge/remove %0=%1%cn again within the next 10 minutes. The time is now [prettytime()].; @eval settimer(%#, remove-%qP, 600, %qN); }; @set %qP=_badges:[remove(xget(%qP, _badges), %qN, |, |)]; @set %qP=_worn-badge:[edit(xget(%qP, _worn-badge), %qN,)]; @set %vS=[edit(%qB, BADGE-, players-)]:[remove(xget(%vS, edit(%qB, BADGE-, players-)), ulocal(f.get-name, %qP), |, |)]; @trigger me/tr.success=%#, cat(You remove, ulocal(f.get-name, %qP, %#)'s, %qN badge.); @trigger me/tr.log=%qP, _sharps-, %#, Removed the %qN badge.; @trigger me/tr.message=%qP, cat(ulocal(f.get-name, %#, %qP) removes your %qN badge., if(not(t(ulocal(f.get-badge, %qP))), Your worn badge has been reset. +badge/wear <badge> to wear one of your other badges.));
...
&c.+badge/retire [v(d.sb)]=$+badge/retire *:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to manage badges.; }; @assert t(setr(B, ulocal(f.find-badge-by-name, %0)))={ @trigger me/tr.error=%#, There is no badge named %0.; }; @eval setq(N, ulocal(f.get-badge-name, %qB)); @set %vS=[edit(%qB, BADGE-, status-)]:[cat(Retired, prettytime(), by, ulocal(f.get-name, %#))]; @trigger me/tr.success=%#, You retire the badge %qN.;

&c.+badge/reactivate [v(d.sb)]=$+badge/reactivate *:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to manage badges.; }; @assert t(setr(B, ulocal(f.find-badge-by-name, %0)))={ @trigger me/tr.error=%#, There is no badge named %0.; }; @eval setq(N, ulocal(f.get-badge-name, %qB)); @wipe %vS/[edit(%qB, BADGE-, status-)]; @trigger me/tr.success=%#, You reactivate the badge %qN.;
...
&f.get-global-noms [v(d.sb)]=rest(finditem(xget(%vS, global-noms-%1), %0-), -)

&f.calc-nom-sharps [v(d.sb)]=switch(ulocal(f.get-total-past-noms, %0, %1), <1, 1, <2, .5, <5, .25, .1)

&c.+nom_player [v(d.sb)]=$+nom* *:@break strmatch(%0, s*); @assert isapproved(%#)={ @trigger me/tr.error=%#, You must be approved to hand out +noms.; }; @eval setq(N, switch(%1, *=*, first(%1, =), %1)); @eval setq(R, switch(%1, *=*, rest(%1, =), Nominated.)); @eval setq(A, switch(%0, */a*, 1, 0)); @eval setq(Q, switch(%0, */q*, 1, */s*, 1, 0)); @break cand(strmatch(%0, /*), not(%qA), not(%qQ)); @assert t(setr(P, ulocal(f.find-player, %qN, %#)))={ @trigger me/tr.error=%#, Could not find a player named '%qN'.; };  @break cand(isstaff(%#), strmatch(%qR, Nominated.))={ @force %#=+noms %qP; };@assert not(strmatch(%qP, %#))={ @trigger me/tr.error=%#, You can't +nom yourself.; }; @eval setq(N, ulocal(f.get-name, %qP, %#)); @assert not(gettimer(%#, nom-%qP))={ @trigger me/tr.error=%#, You can't +nom the same person more than once per day. You will be able to +nom %qN again in [getremainingtime(%#, nom-%qP)].; }; @assert t(setr(S, ulocal(f.calc-nom-sharps, %#, %qP)))={ @trigger me/tr.error=%#, Could not figure out how many sharps to award.; }; @trigger me/tr.log=%qP, _nom-, if(%qA, Anonymous, %#), %qR; @trigger me/tr.log=%#, _nom-, %#, strcat(if(%qA, Anonymously nominated, Nominated), %b, %qN, if(strmatch(%qR, Nominated.), ., %bfor: %qR)); @trigger me/tr.log=%qP, _sharps-, if(%qA, Anonymous, %#), cat(Awarded %qS, plural(%qS, sharp, sharps) via +nom.); @set %qP=_total-sharps:[add(%qS, xget(%qP, _total-sharps))]; @eval setq(L, ulocal(f.get-total-past-noms, %#, %qP)); @set %qP=_sharps:[add(%qS, xget(%qP, _sharps))]; @set %#=_past-noms:[setunion(setdiff(xget(%#, _past-noms), %qP-%qL), %qP-[inc(%qL)])]; @eval setq(G, ulocal(f.get-global-noms, %#, given)); @set %vS=global-noms-given:[setunion(setdiff(xget(%vS, global-noms-given), %#-%qG), %#-[inc(%qG)])]; @eval setq(G, ulocal(f.get-global-noms, %qP, received)); @set %vS=global-noms-received:[setunion(setdiff(xget(%vS, global-noms-received), %qP-%qG), %qP-[inc(%qG)])]; @eval settimer(%#, nom-%qP, 86400); @trigger me/tr.success=%#, squish(strcat(You, %b, itemize(cat(if(%qA, anonymously), if(%qQ, quietly))) +nom, %b, ulocal(f.get-name, %qP, %#) for %qS Sharps, if(strmatch(%qR, Nominated.), ., %bwith the following comment: %qR))); @if not(%qQ)={ @trigger me/tr.message=%qP, strcat(if(%qA, An anonymous player, ulocal(f.get-name, %#, %qP)) +noms you for %qS Sharps, if(strmatch(%qR, Nominated.), ., %bwith the following comment: %qR)); }; @assert t(v(d.noms-channel)); @trigger me/tr.alert-to-channel=v(d.noms-channel), %#, strcat(capstr(trim(squish(cat(itemize(cat(if(%qA, anonymously), if(%qQ, quietly))), nominated)))), %b, %qN for %qS Sharps, if(strmatch(%qR, Nominated.), ., %bwith the following comment: %qR));
