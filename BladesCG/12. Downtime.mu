...
+health
	+harm <injury>
	+harm <#> <injury>
*	+harm/clear <player>=<# or all> (staff-only) - clear player's harm after they've hit max?
+stress/+trauma
	+stress/gain
	+stress/gain <#>
	+stress/clear
...
@daily [v(d.cg)]=@dolist xget(%vD, d.logs-to-clear)={ @trigger me/tr.clean-old-player-logs=##; };
...
&c.+dt/spend [v(d.cg)]=$+dt/spend *=*:@break cand(not(isnum(%0)), isstaff(%#)); @eval setq(D, ulocal(f.get-player-stat, %#, downtime)); @assert cand(isnum(%0), gt(%0, 0), lte(%0, %qD))={ @trigger me/tr.error=%#, %0 must be a number between 1 and your current downtime%, %qD.; }; @assert t(%1)={ @trigger me/tr.error=%#, You must specify a reason for spending this downtime.; }; @set %#=[ulocal(f.get-stat-location-on-player, downtime)]:[sub(%qD, %0)]; @trigger me/tr.log-downtime=%#, %#, Spent %0 for '%1'.; @trigger me/tr.success=%#, You spend %0 downtime for '%1'.; @trigger me/tr.alert-to-monitor=%#, Spent %0 downtime for '%1'.;

&c.+dt/spend_staff [v(d.cg)]=$+dt/spend *=*:@break isnum(%0); @assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to modify a player's stats.; }; @assert t(setr(P, ulocal(f.find-player, %0, %#)))={ @trigger me/tr.error=%#, Could not find a player named '%0'.; }; @eval setq(D, ulocal(f.get-player-stat, %qP, downtime)); @eval setq(V, if(isnum(first(%1)), first(%1), 0)); @eval strcat(setq(R, if(isnum(first(%1)), rest(%1), %1)), setq(R, squish(trim(switch(%qR, for *, rest(%qR), %qR))))); @assert t(%qR)={ @trigger me/tr.error=%#, Can't figure out what your reason for spending this downtime was.; }; @assert cand(isnum(%qV), gt(%qV, 0), lte(%qV, %qD))={ @trigger me/tr.error=%#, %qV must be a number between 1 and the player's current downtime%, %qD.; }; @set %qP=[ulocal(f.get-stat-location-on-player, downtime)]:[sub(%qD, %qV)]; @trigger me/tr.log-downtime=%qP, %#, Spent %qV for '%qR'.; @trigger me/tr.stat-setting-messages=cat(You spend %qV of, ulocal(f.get-name, %qP, %#)'s downtime for '%qR'.), cat(ulocal(f.get-name, %#, %qP) spends %qV of your downtime for '%qR'. You now have, ulocal(f.get-player-stat, %qP, downtime) downtime.), %qP, %#, Downtime;
