...
&c.+rep/award [v(d.cg)]=$+rep/award *=*:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to modify a player's stats.; }; @assert t(setr(P, ulocal(f.find-player, %0, %#)))={ @trigger me/tr.error=%#, Could not find a player named '%0'.; }; @assert t(setr(C, ulocal(f.get-player-stat, %qP, crew object)))={ @trigger me/tr.error=%#, ulocal(f.get-name, %qP) is not part of a crew and cannot gain rep.; }; @eval setq(V, if(isint(first(%1)), first(%1), 0)); @eval strcat(setq(R, if(isint(first(%1)), rest(%1), %1)), setq(R, squish(trim(switch(%qR, for *, rest(%qR), %qR))))); @assert t(%qR)={ @trigger me/tr.error=%#, Can't figure out what your reason for granting this downtime was.; }; @eval setq(M, sub(ulocal(f.get-max-rep, %qC), setr(A, ulocal(f.get-player-stat, %qC, Rep)))); @assert t(%qM)={ @trigger me/tr.error=%#, This crew cannot receive any more rep - they need to buy up their Tier!; }; @assert cand(gt(%qV, 0), lte(%qV, %qM))={ @trigger me/tr.error=%#, %qV must be a number between 1 and %qM.; }; @set %qC=[ulocal(f.get-stat-location-on-player, rep)]:[add(%qA, %qV)]; @trigger me/tr.log=%qP, _rep-, %#, Awarded %qV rep for '%qR'.; @trigger me/tr.stat-setting-messages=cat(You award, ansi(h, ulocal(f.get-crew-name, %qC)), %qV, rep for '%qR'.), ulocal(f.get-name, %#, %qP) awards your crew %qV rep for '%qR'., %qP, %#, Reputation;
