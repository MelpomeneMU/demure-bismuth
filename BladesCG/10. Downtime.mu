...
+heat
	...
	+heat/gain <#>=<reason>
+rep
	+rep/withdraw <#>=<reason> - turn a point of Rep into a point of Downtime
...
@daily [v(d.cg)]=@trigger me/tr.award-downtime;
...
&layout.player-coin [v(d.cgf)]=strcat(ulocal(layout.coin, %0, %1), %r, ulocal(layout.crew-coin, setr(C, ulocal(f.get-player-stat, %0, crew object)), %1), if(t(setr(L, ulocal(layout.crew-coin-buffs, %qC))), strcat(%r, formattext(strcat(%r, After-score crew coin buffs:, %r%r, iter(%qL, cat(*, itext(0)), |, %r)), 0, %1))), setq(L, ulocal(f.get-last-X-logs, %0, _coin-)), if(t(%qL), strcat(%r, divider(Last 10 coin logs, %1), %r, formattext(iter(%qL, ulocal(layout.log, xget(%0, itext(0))),, %r), 0, %1))), setq(L, ulocal(f.get-last-X-logs, setr(C, ulocal(f.get-player-stat, %0, crew object)), _crew-coin-)), if(t(%qL), strcat(%r, divider(Last 10 crew coin logs, %1), %r, formattext(iter(%qL, ulocal(layout.log, xget(%qC, itext(0))),, %r), 0, %1))), %r, footer(, %1))

&layout.player-stress [v(d.cgf)]=strcat(ulocal(layout.stress, %0, %1), setq(L, ulocal(f.get-last-X-logs, %0, _stress-)), if(t(%qL), strcat(%r, divider(Last 10 stress logs, %1), %r, formattext(iter(%qL, ulocal(layout.log, xget(%0, itext(0))),, %r), 0, %1))), %r, footer(, %1))

&layout.crew-coin-buffs [v(d.cgf)]=squish(trim(strcat(if(ulocal(f.has-mapped-stat, %0, Covert Drop), Covert Drop: +2 coin for scores involving espionage or sabotage.), |, if(ulocal(f.has-mapped-stat, %0, Loyal Fence), Loyal Fence: +2 coin for scores involving burglary or robbery.), |, if(ulocal(f.has-mapped-stat, %0, Envoy), Envoy: +2 coin for scores involving high-class clients.), |, if(ulocal(f.has-mapped-stat, %0, Fixer), Fixer: +2 coin for scores involving low-class clients.), |, if(ulocal(f.has-mapped-stat, %0, Street Fence), Street Fence: +2 coin for scores involving low-class targets.), |, if(ulocal(f.has-mapped-stat, %0, Terrorized Citizens), Terrorized Citizens: +2 coin for scores involving battle or extortion.), |, if(ulocal(f.has-mapped-stat, %0, Offertory), Offertory: +2 coin for scores that involve occult operations.), |, if(ulocal(f.has-mapped-stat, %0, Local Graft), Local Graft: +2 coin for scores involving a show of force or socializing.), |, if(ulocal(f.has-mapped-stat, %0, Surplus Cache), Surplus Cache: +2 coin for scores that involve product sale or supply.), |, if(ulocal(f.has-mapped-stat, %0, Luxury Fence), Luxury Fence: +2 coin for scores involving high-class targets.), setq(M, Roll Tier and earn coin equal to the highest result%, minus Heat.), |, if(ulocal(f.has-mapped-stat, %0, Vice Den), Vice Den: %qM), |, if(ulocal(f.has-mapped-stat, %0, Protection Racket), Protection Racket: %qM), |, if(ulocal(f.has-mapped-stat, %0, Fighting Pit), Fighting Pit: %qM), |, if(ulocal(f.has-mapped-stat, %0, Foreign Market), Foreign Market: %qM), |, if(ulocal(f.has-mapped-stat, %0, Drug Den), Drug Den: %qM), |, if(ulocal(f.has-mapped-stat, %0, Gambling Den), Gambling Den: %qM), |, if(ulocal(f.has-mapped-stat, %0, Side Business), Side Business: %qM)), b, |), |)

&layout.heat [v(d.cgf)]=strcat(ulocal(layout.crew-heat, setr(C, ulocal(f.get-player-stat, %0, crew object)), %1), setq(L, ulocal(f.get-last-X-logs, %qC, _heat-)), if(t(%qL), strcat(%r, divider(Last 10 heat logs, %1), %r, formattext(iter(%qL, ulocal(layout.log, xget(%qC, itext(0))),, %r), 0, %1))), %r, footer(, %1))
...
&c.+heat [v(d.cg)]=$+heat: @pemit %#=ulocal(layout.heat, %#, %#);

&c.+heat_staff [v(d.cg)]=$+heat *:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to view a player's stats.; }; @assert t(setr(P, ulocal(f.find-player, %0, %#)))={ @trigger me/tr.error=%#, Could not find a player named '%0'.; }; @pemit %#=ulocal(layout.heat, %qP, %#);

&c.+stress [v(d.cg)]=$+stress: @pemit %#=ulocal(layout.player-stress, %#, %#);

&c.+stress_staff [v(d.cg)]=$+stress *:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to view a player's stats.; }; @assert t(setr(P, ulocal(f.find-player, %0, %#)))={ @trigger me/tr.error=%#, Could not find a player named '%0'.; }; @pemit %#=ulocal(layout.player-stress, %qP, %#);
...
&c.+dt/buy [v(d.cg)]=$+dt/buy*: @eval setq(V, switch(%0, %b*, trim(%0), * *, rest(%0), 1)); @assert cand(isint(%qV), gt(%qV, 0), lte(%qV, 10))={ @trigger me/tr.error=%#, %qV must be a number between 1 and 10.; }; @assert gte(setr(C, ulocal(f.get-player-stat-or-zero, %#, coin)), %qV)={ @trigger me/tr.error=%#, You have %qC coin%, not enough to buy %qV downtime.; }; @assert gettimer(%#, buy-dt, %0)={ @trigger me/tr.message=%#, You're about to buy %qV downtime for %qV coin. There is no way to convert downtime back into coin. Are you sure? If so%, hit %ch+dt/buy%0%cn again within the next 10 minutes. The time is now [prettytime()].; @eval settimer(%#, buy-dt, 600, %0); }; @set %#=[ulocal(f.get-stat-location-on-player, downtime)]:[add(ulocal(f.get-player-stat-or-zero, %#, downtime), %qV)]; @set %#=[ulocal(f.get-stat-location-on-player, coin)]:[sub(%qC, %qV)]; @trigger me/tr.log-downtime=%#, %#, Bought %qV downtime with coin.; @trigger me/tr.log=%#, _coin-, %#, Spent %qV coin on downtime.; @trigger me/tr.success=%#, You buy %qV downtime for %qV coin. You now have [ulocal(f.get-player-stat, %#, downtime)] downtime and [ulocal(f.get-player-stat-or-zero, %#, coin)] coin.;
...
@@ %0: player
@@ %1: message
&tr.alert-to-monitor [v(d.cg)]=@cemit [xget(%vG, d.monitor-channel)]=[cat(ansi(xh, extract(prettytime(), 1, 2, /)), ansi(<#777777>, rest(prettytime())), ulocal(f.get-name, %0):, %1)];

&layout.gain-stress [v(d.cgf)]=strcat(alert(Stress), %b, ulocal(f.get-name, %0) gains %1 stress, if(t(%4), %bbecause '%4'), %, bringing, %b, poss(%0), %b, total to %2/%3., if(eq(%2, %3), cat(, capstr(subj(%0)), plural(%0, is, are), out of the scene and must take a trauma before, subj(%0), can return to play.)))
...
&layout.overindulge-job [v(d.cgf)]=cat(ulocal(f.get-name, %0), overindulged, poss(%0), vice of, ulocal(f.get-player-stat, %0, vice), by, %1, points.%R%R%TWhen you overindulge%, you make a bad call because of your vice - in acquiring it or while under its influence. To bring the effect of this bad decision into the game%, select an overindulgence from the list:%R%R%T- %chAttract Trouble.%cn Select or roll an additional entanglement.%R%T- Brag about your exploits. +2 heat for your crew.%R%T- %chLost.%cn Your character vanishes for a few weeks. Play a different character until this one returns from their bender. When your character returns%, they've also healed any harm they had.%R%T- %chTapped.%cn Your current purveyor cuts you off. Find a new source for your vice.%R)

&layout.max-trauma-job [v(d.cgf)]=cat(ulocal(f.get-name, %0), has hit, poss(%0), maximum trauma level.%R%R%TAt this point, there are a few options:%R%R%T- %chRetirement.%cn Your character lives out, poss(%0), days off-screen with a lifestyle of, ulocal(f.get-lifestyle-desc, %0). You may transfer, poss(%0), advancements to a new PC according to our policies.%R%T- %chBecome a Ghost.%cn Your character has died., capstr(poss(%0)), playbook changes to Ghost and, subj(%0), plural(%0, begins, begin), a new and spooky chapter of, poss(%0), life.%R%T- %chImprisonment.%cn Your character sacrifices, obj(%0)self, for the good of the crew. Your crew's Wanted Level drops by 1 and all their Heat is cleared. Your character never returns%, but you may transfer, poss(%0), advancements to a new PC according to our policies.%R)
...
&c.+stress/gain [v(d.cg)]=$+stress/gain *=*: @eval setq(M, ulocal(f.get-max-stress, %#)); @assert cand(isint(%0), gt(%0, 0), lte(%0, %qM))={ @trigger me/tr.error=%#, '%0' must be a number between 1 and %qM.; }; @eval setq(X, ulocal(f.get-max-trauma, %#)); @eval setq(T, ulocal(f.get-player-stat, %#, Traumas)); @eval setq(C, ulocal(f.get-player-stat, %#, Stress)); @assert not(cand(gte(%qC, %qM), eq(words(%qT, |), %qX)))={ @trigger me/tr.error=%#, You have reached your limit for all stress and trauma.; }; @assert lte(setr(N, add(%qC, %0)), %qM)={ @trigger me/tr.error=%#, You have [sub(%qM, %qC)] stress remaining. %0 is too high.; }; @assert cor(lt(%qN, %qM), gettimer(%#, stress-limit, %0=%1))={ @eval settimer(%#, stress-limit, 600, %0=%1); @trigger me/tr.message=%#, Taking this stress would put you at your maximum of %qM stress. You will immediately be out of the scene and [if(eq(words(%qT, |), %qX), will need to resolve your character's situation with staff, will have to take a trauma)] before you can return to play. Are you sure you want to take this stress? Hit %ch+stress/gain %0[if(t(%1), =%1)]%cn within the next 10 minutes to confirm. The time is now [prettytime()].; }; @set %#=[ulocal(f.get-stat-location-on-player, stress)]:%qN; @trigger me/tr.remit-or-pemit=%L, ulocal(layout.gain-stress, %#, %0, %qN, %qM, %1), %#; @trigger me/tr.alert-to-monitor=%#, strcat(Gained %0 stress, if(t(%1), %bbecause '%1'), .); @trigger me/tr.log=%#, _stress-, %#, strcat(Gained %0 stress, if(t(%1), %bbecause '%1'), .); @assert neq(%qN, %qM)={ @set %#=[ulocal(f.get-stat-location-on-player, needs trauma)]:1; @if eq(words(%qT, |), %qX)={ @trigger %vA/trig_create=%#, xget(%vG, d.characters-bucket), 1, cat(Max Trauma:, ulocal(f.get-name, %#)), ulocal(layout.max-trauma-job, %#); }; };
...
&c.+rep/award [v(d.cg)]=$+rep/award *=*:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to modify a player's stats.; }; @assert t(setr(P, ulocal(f.find-player, %0, %#)))={ @trigger me/tr.error=%#, Could not find a player named '%0'.; }; @assert t(setr(C, ulocal(f.get-player-stat, %qP, crew object)))={ @trigger me/tr.error=%#, ulocal(f.get-name, %qP) is not part of a crew and cannot gain rep.; }; @eval setq(V, if(isint(first(%1)), first(%1), 0)); @eval strcat(setq(R, if(isint(first(%1)), rest(%1), %1)), setq(R, squish(trim(switch(%qR, for *, rest(%qR), %qR))))); @assert t(%qR)={ @trigger me/tr.error=%#, Can't figure out what your reason for granting this downtime was.; }; @eval setq(M, sub(12, setr(A, ulocal(f.get-player-stat, %qC, Rep)))); @assert t(%qM)={ @trigger me/tr.error=%#, This crew cannot receive any more rep, they have no room left on their rep bar.; }; @assert cand(gt(%qV, 0), lte(%qV, %qM))={ @trigger me/tr.error=%#, %qV must be a number between 1 and %qM.; }; @set %qC=[ulocal(f.get-stat-location-on-player, rep)]:[add(%qA, %qV)]; @trigger me/tr.log=%qP, _rep-, %#, Awarded %qV rep for '%qR'.; @trigger me/tr.stat-setting-messages=cat(You award, ansi(h, ulocal(f.get-crew-name, %qC)), %qV, rep for '%qR'.), ulocal(f.get-name, %#, %qP) awards your crew %qV rep for '%qR'., %qP, %#, Reputation;

&c.+rep/withdraw [v(d.cg)]=$+rep/withdraw *=*:@assert t(setr(C, ulocal(f.get-player-stat, %#, crew object)))={ @trigger me/tr.error=%#, You're not in a crew and can't withdraw rep.; }; @eval setq(V, %0); @eval setq(R, %1); @assert t(%qR)={ @trigger me/tr.error=%#, Can't figure out what your reason for withdrawing this rep was.; }; @assert cand(isint(%qV), gt(%qV, 0), lte(%qV, 4))={ @trigger me/tr.error=%#, %qV must be a number between 1 and 4.; }; @assert gte(setr(S, ulocal(f.get-player-stat-or-zero, %qC, rep)), %qV)={ @trigger me/tr.error=%#, Your crew doesn't have %qV rep to withdraw as downtime %qS.; }; @set %qC=[ulocal(f.get-stat-location-on-player, rep)]:[sub(%qS, %qV)]; @set %#=[ulocal(f.get-stat-location-on-player, downtime)]:[add(ulocal(f.get-player-stat, %#, downtime), %qV)]; @trigger me/tr.log=%qC, _rep-, %#, Withdrew %qV rep as downtime.; @trigger me/tr.stat-setting-messages=cat(You withdraw, ansi(h, ulocal(f.get-crew-name, %qC)), %qV, rep as downtime for '%qR'.), ulocal(f.get-name, %#) withdraws %qV rep as downtime for '%qR'., %#, %#, Reputation; @trigger me/tr.remit-or-pemit=%l, cat(alert(Rep), ulocal(f.get-name, %#), withdrew %qV rep as downtime for '%qR'.), %#;

&c.+rep/spend [v(d.cg)]=$+rep/spend *=*:@break cand(not(isint(%0)), isstaff(%#)); @assert t(setr(C, ulocal(f.get-player-stat, %#, crew object)))={ @trigger me/tr.error=%#, You're not in a crew and can't spend rep.; }; @eval setq(V, %0); @eval setq(R, %1); @assert t(%qR)={ @trigger me/tr.error=%#, Can't figure out what your reason for spending this rep was.; }; @assert cand(isint(%qV), gt(%qV, 0), lte(%qV, 12))={ @trigger me/tr.error=%#, %qV must be a number between 1 and 12.; }; @assert gte(setr(O, ulocal(f.get-player-stat-or-zero, %qC, rep)), %qV)={ @trigger me/tr.error=%#, Your crew has %qO rep%, not enough to spend %qV.; }; @set %qC=[ulocal(f.get-stat-location-on-player, rep)]:[sub(%qO, %qV)]; @trigger me/tr.log=%#, _rep-, %#, Spent %qV rep for '%qR'.; @trigger me/tr.stat-setting-messages=cat(You spend, ansi(h, ulocal(f.get-crew-name, %qC)), %qV, rep for '%qR'.), ulocal(f.get-name, %#) spends %qV rep for '%qR'., %#, %#, Reputation; @trigger me/tr.remit-or-pemit=%l, cat(alert(Rep), ulocal(f.get-name, %#), spent %qV rep for '%qR'.), %#;

&c.+rep/spend_staff [v(d.cg)]=$+rep/spend *=*:@break isint(%0); @assert isstaff(%#)={ @trigger me/tr.error=%#, Only staff can change other players' stats.; }; @assert t(setr(P, ulocal(f.find-player, %0, %#)))={ @trigger me/tr.error=%#, Could not find a player named '%0'.; }; @eval setq(N, ulocal(f.get-name, %qP, %#)); @assert t(setr(C, ulocal(f.get-player-stat, %qP, crew object)))={ @trigger me/tr.error=%#, %qN is not in a crew and can't spend crew rep.; }; @eval setq(V, if(isint(first(%1)), first(%1), 0)); @eval strcat(setq(R, if(isint(first(%1)), rest(%1), %1)), setq(R, squish(trim(switch(%qR, for *, rest(%qR), %qR))))); @assert t(%qR)={ @trigger me/tr.error=%#, Can't figure out what your reason for spending this rep was.; }; @assert cand(isint(%qV), gt(%qV, 0), lte(%qV, 12))={ @trigger me/tr.error=%#, %qV must be a number between 1 and 12.; }; @assert gte(setr(S, ulocal(f.get-player-stat-or-zero, %qC, rep)), %qV)={ @trigger me/tr.error=%#, This crew doesn't have %qV rep - they only have %qS rep.; }; @set %qC=[ulocal(f.get-stat-location-on-player, rep)]:[sub(%qS, %qV)]; @trigger me/tr.log=%qC, _rep-, %#, Spent %qV rep for '%qR'.; @trigger me/tr.stat-setting-messages=cat(You spend %qV rep from, ansi(h, ulocal(f.get-crew-name, %qC)) for '%qR'.), ulocal(f.get-name, %#) spends %qV of your crew's rep for '%qR'., %qP, %#, Reputation;
