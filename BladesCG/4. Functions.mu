...
@@ %1: player being edited
@@ %2: stat to be edited
&f.is-allowed-to-edit-stat [v(d.cgf)]=cor(not(hasattr(%1, if(ulocal(f.is-crew-stat, %2), _stat.crew_locked, _stat.locked))), t(member(xget(%vG, d.stats_editable_after_chargen), %2, |)), ulocal(f.is-allowed-to-break-stat-setting-rules, %0, %1))
...
&f.get-abilities [v(d.cgf)]=strcat(setq(S,), null(iter(xget(%vG, d.abilities), setq(S, strcat(%qS, |, xget(%vG, itext(0)))))), squish(trim(%qS, b, |), |))

&f.get-upgrades-with-boxes [v(d.cgf)]=strcat(setq(S,), null(iter(xget(%vG, d.upgrades), setq(S, strcat(%qS, |, xget(%vG, itext(0)))))), squish(trim(%qS, b, |), |))

@@ %0 - list of player's upgrades
@@ %1 - list of player's expected crew upgrades
&f.get-extra-crew-upgrades [v(d.cgf)]=strcat(setq(S, setq(R,)), null(iter(d.upgrades.lair d.upgrades.quality d.upgrades.training, setq(S, strcat(%qS, |, xget(%vG, itext(0)))))), setq(S, squish(trim(%qS|%1, b, |), |)), null(iter(%0, if(not(t(ulocal(f.find-tickable, %qS, trim(last(itext(0), \]))))), setq(R, strcat(%qR, |, itext(0)))), |, |)), squish(trim(%qR, b, |), |))

&f.get-upgrades [v(d.cgf)]=strcat(setq(S,), null(iter(xget(%vG, d.upgrades), setq(S, strcat(%qS, |, iter(xget(%vG, itext(0)), trim(last(itext(0), \])), |, |))))), squish(trim(%qS, b, |), |))
...
&f.get-addable-stats [v(d.cgf)]=xget(%vG, if(isstaff(%1), d.addable-stats, d.cg-addable-stats))

&f.get-choice-list [v(d.cgf)]=strcat(setq(0, if(t(%0), remove(ulocal(f.list-valid-values, %0, %1), any unrestricted text, |), ulocal(f.get-choices, %1))), case(1, t(member(xget(%vG, d.playbook-stats), %0, |)), setdiff(%q0, ulocal(f.list-restricted-values, Playbook, %1), |), t(member(xget(%vG, d.crew-type-stats), %0, |)), setdiff(%q0, ulocal(f.list-restricted-values, Crew Type, %1), |), %q0))

@@ %0: player
&f.get-choices [v(d.cgf)]=strcat(squish(trim(iter(ulocal(f.get-player-bio-fields, %0), if(hasattr(%vG, strcat(d.value., ulocal(f.get-stat-location, itext(0)))), itext(0)), |, |), b, |), |), |, xget(%vG, d.choose-sections))

&f.get-choosable-stats [v(d.cgf)]=xget(%vG, d.choosable-stats)

&f.is-stat [v(d.cgf)]=strcat(setq(N, ulocal(f.resolve-stat-name, %0)), finditem(ulocal(f.list-stats, %1, %2), %qN, |))
...
@@ xget(%vG, d.staff-only-stats)
&f.list-stats [v(d.cgf)]=strcat(setq(D, if(ulocal(f.is-allowed-to-break-stat-setting-rules, %1, %0),, xget(%vG, d.staff-only-stats))), diffset(ulocal(f.list-all-stats, %0), %qD, |, |))

&f.is-full-list-stat [v(d.cgf)]=t(finditem(xget(%vG, d.stats-where-player-gets-entire-list), %0, |))

&f.get-field-note [v(d.cgf)]=itemize(default(strcat(%vG, /, d.value., ulocal(f.get-stat-location, %0)), switch(%0, Name, Your full IC name, Alias, Your street alias, Look, Your short-desc, No limits, Vice Purveyor, A specific business or individual who helps you fulfill your Vice. This is free-text.)), |, or)

&f.get-layout-bio-stats [v(d.cgf)]=strcat(setq(L, ulocal(f.get-player-bio-fields, %0)), null(iter(xget(%vG, d.manual-bio-stats), setq(L, remove(%qL, itext(0), |)), |)), %qL)

&f.get-player-bio-fields [v(d.cgf)]=if(ulocal(f.is_expert, %0), xget(%vG, d.expert_bio), strcat(setq(P, xget(%0, ulocal(f.get-stat-location-on-player, Playbook))), setq(F, xget(%vG, d.bio)), setq(F, strcat(%qF, |, xget(%vG, d.bio.%qP))), setq(F, setdiff(%qF, xget(%vG, d.bio.%qP.exclude), |, |)), squish(trim(%qF, b, |), |)))

&f.get-player-abilities [v(d.cgf)]=strcat(setq(P, xget(%0, ulocal(f.get-stat-location-on-player, Playbook))), setq(F, default(%vG/d.abilities.[edit(%qP, %b, _)], ulocal(f.get-abilities))), squish(trim(%qF, b, |), |))

&f.get-player-crew-abilities [v(d.cgf)]=strcat(setq(O, xget(%0, ulocal(f.get-stat-location-on-player, crew object))), setq(T, ulocal(f.get-player-stat, %qO, crew type)), setq(F, default(%vG/d.crew_abilities.[edit(%qT, %b, _)], ulocal(f.get-crew-abilities))), squish(trim(%qF, b, |), |))
...
&f.get-playbook-default [v(d.cgf)]=strcat(setq(F, if(cand(t(%0), switch(%1, XP Triggers, 1, Friends, 1, Gear, 1, Abilities, 1, 0)), xget(%vG, strcat(d., ulocal(f.get-stat-location, %1), ., %0)))), if(switch(%1, Abilities, 1, 0), first(%qF, |), %qF))

&f.get-crew-default [v(d.cgf)]=strcat(setq(F, if(strmatch(%1, Favorite), ulocal(f.get-first-default, %2, Contacts), if(cand(t(%0), switch(%1, Crew XP Triggers, 1, Crew Abilities, 1, Contacts, 1, Favorite, 1, 0)), xget(%vG, strcat(d., ulocal(f.get-stat-location, %1), ., %0))))), if(switch(%1, Crew Abilities, 1, Favorite, 1, 0), first(%qF, |), %qF))
...
@@ %2: Is crew stat
@@ %3: Crew object
&f.get-stat-default [v(d.cgf)]=if(%2, ulocal(f.get-crew-default, xget(%3, ulocal(f.get-stat-location-on-player, Crew Type)), %1, %3), switch(%1, Ally, ulocal(f.get-first-default, %0, Friends), Rival, ulocal(f.get-last-default, %0, Friends), Standard Gear, xget(%vG, d.standard_gear), ulocal(f.get-playbook-default, xget(%0, ulocal(f.get-stat-location-on-player, Playbook)), %1)))

&f.get-crew-abilities [v(d.cgf)]=strcat(setq(S,), null(iter(xget(%vG, d.crew_abilities), setq(S, strcat(%qS, |, xget(%vG, itext(0)))))), squish(trim(%qS, b, |), |))

&f.is-crew-stat [v(d.cgf)]=cor(cand(t(setr(S, finditem(ulocal(f.list-crew-stats), %0, |))), strmatch(%qS, %0)), t(member(xget(%vG, d.map-list), %0, |)))

&f.list-crew-stats [v(d.cgf)]=strcat(xget(%vG, d.crew_bio), |, xget(%vG, d.crew-stats))

th ulocal(v(d.cgf)/f.list-crew-stats)

&f.resolve-stat-name [v(d.cgf)]=strcat(setq(0, setq(1,)), null(iter(lattr(%vG/d.alias.*), if(cand(not(t(%q0)), t(finditem(setr(1, xget(%vG, itext(0))), %0, |))), setq(0, last(%q1, |))))), if(t(%q0), %q0, %0))
...
&f.get-random-name-and-job [v(d.cgf)]=strcat(pickrand(xget(%vG, d.random.name), |), %,%b, art(setr(J, pickrand(xget(%vG, d.random.job), |))), %b, %qJ)
...
&f.get-stats [v(d.cgf)]=squish(trim(strcat(xget(%vG, d.scoundrel-stats), |, setdiff(ulocal(f.get-player-bio-fields, %0), Crew, |, |), |, setdiff(xget(%vG, d.expert_bio), Crew, |, |), |, if(t(ulocal(f.get-player-stat, %0, crew object)), ulocal(f.get-crew-stats))), b, |), |)

&f.get-crew-stats [v(d.cgf)]=xget(%vG, d.crew_bio)|Favorite
...
&f.is-attribute [v(d.cgf)]=finditem(xget(%vG, d.attributes), %0, |)

&f.get-player-attribute [v(d.cgf)]=ladd(iter(xget(%vG, d.actions.%1), t(ulocal(f.get-player-stat-or-zero, %0, itext(0))), |,))

&f.list-actions [v(d.cgf)]=xget(%vG, d.actions)

&f.list-restricted-values [v(d.cgf)]=xget(%vG, if(ulocal(f.is-action, %0), d.restricted.action, ulocal(f.get-stat-location, d.restricted.%0)))

&f.list-sheet-sections [v(d.cgf)]=xget(%vG, d.sheet-sections)
...
&f.get-full-list-stat-values [v(d.cgf)]=iter(lattr(strcat(%vG, /, d., ulocal(f.get-stat-location, %0), ., *)), title(lcstr(last(itext(0), .))),, |)

&f.list-values [v(d.cgf)]=case(1, t(ulocal(f.is-action, %0)), xget(%vG, d.value.action), ulocal(f.is-full-list-stat, %0), ulocal(f.get-full-list-stat-values, %0, %1), strmatch(Ally, %0), remove(ulocal(f.get-player-stat, %1, friends), ulocal(f.get-player-stat, %1, rival), |), strmatch(Rival, %0), remove(ulocal(f.get-player-stat, %1, friends), ulocal(f.get-player-stat, %1, ally), |), strmatch(Favorite, %0), ulocal(f.get-player-stat, %1, contacts), strmatch(Abilities, %0), ulocal(f.get-abilities), t(finditem(Crew Ability|Crew Abilities, %0, |)), ulocal(f.get-crew-abilities), strmatch(%0, Upgrade*), ulocal(f.get-upgrades), xget(%vG, ulocal(f.get-stat-location, d.value.%0)))
...
&f.find-cohort-stat-name [v(d.cgf)]=finditem(xget(%vG, d.cohort.stats), %0, |)

&f.is-addable-cohort-stat [v(d.cgf)]=t(member(xget(%vG, d.cohort.addable_stats), %0, |))
...
&f.list-cohort-stat-pretty-values [v(d.cgf)]=xget(%vG, strcat(d.value., if(strmatch(%1, Vehicle), vehicle_), ulocal(f.get-stat-location, %0)))
...
&f.get-all-factions [v(d.cgf)]=iter(xget(%vG, d.factions), xget(%vG, itext(0)),, |)

@@ %0: Crew object
@@ %1: Optional - question to skip
&f.get-total-faction-coin [v(d.cgf)]=ladd(iter(xget(%vG, d.faction.questions), if(not(t(member(%1|Friendly|Unfriendly, itext(0), |))), rest(ulocal(f.get-player-stat, %0, strcat(faction., itext(0))), |)), |))
...
&f.get-total-advancements [v(d.cgf)]=ladd(iter(setdiff(xget(%vG, d.xp_tracks), Crew|Untracked, |, |), ulocal(f.get-advancements, %0, itext(0), %1), |))
...
&f.get-map-key [v(d.cgf)]=default(strcat(%0, /, ulocal(f.get-stat-location-on-player, Map %1)), extract(xget(%vG, strcat(d.map., ulocal(f.get-player-stat, %0, Crew Type))), switch(%1, A*, rest(%1, A), B*, add(5, if(gt(setr(I, rest(%1, B)), 2), -1, 0), %qI), add(9, rest(%1, C))), 1, |))

&f.get-map-h-join [v(d.cgf)]=edit(mid(strip(edit(xget(%vG, strcat(d.map-h-joins., ulocal(f.get-player-stat, %0, Crew Type))), %b, _), ABC12345), dec(switch(%1, A*, rest(%1, A), B*, add(5, rest(%1, B)), add(10, rest(%1, C)))), 1), _, %b)

&f.get-map-v-join [v(d.cgf)]=mid(xget(%vG, strcat(d.map-v-joins., ulocal(f.get-player-stat, %0, Crew Type))), dec(switch(%1, A*, rest(%1, A), B*, add(5, rest(%1, B)), add(10, rest(%1, C)))), 1)

@@ %0: crew object
@@ %1: stat name
&f.has-mapped-stat [v(d.cgf)]=t(ladd(cat(iter(xget(%vG, d.map-list), strmatch(ulocal(f.get-player-stat, %0, itext(0)), %1), |), iter(ulocal(f.get-player-stat, %0, Claims), strmatch(itext(0), %1), |))))
...
&f.is-probationary-member [v(d.cgf)]=cand(ulocal(f.is-crew-member, %0, %1), not(ulocal(f.is-founding-member, %0, %1)), if(ulocal(f.is-crew-approved, %1), lt(ulocal(f.days-since-joined-crew, %0, %1), xget(%vG, d.crew-probationary-period)), 0))
...
&f.is-idle-member [v(d.cgf)]=cand(ulocal(f.is-crew-member, %0, %1), gt(ulocal(f.days-since-last-connected, %0), xget(%vG, d.crew-idle-before-bootable)), not(strmatch(%0, %1)))
