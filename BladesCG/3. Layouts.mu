...
&layout.health [v(d.cgf)]=strcat(divider(Health, %1), setq(4, ulocal(layout.3health, %0, %1, 4, %2)), setq(3, ulocal(layout.3health, %0, %1, 3, %2)), setq(2, ulocal(layout.2health, %0, %1, 2, %2)), setq(1, ulocal(layout.2health, %0, %1, 1, %2)), if(t(%q4), strcat(%r, %q4)), if(t(%q3), strcat(%r, %q3)), if(t(%q2), strcat(%r, %q2)), if(t(%q1), strcat(%r, %q1)), %r, multicol(strcat(if(cor(t(%q4), t(%q3), t(%q2), t(%q1)), |||||), |, Healing clock:, %b, default(%0/_health-clock, 0), /, ulocal(f.get-max-healing-clock, %0), |, %[, if(t(xget(%0, ulocal(f.get-stat-location-on-player, armor))), X, %b), %] Armor, |, %[, if(t(xget(%0, ulocal(f.get-stat-location-on-player, heavy armor))), X, %b), %] Heavy, |, %[, if(t(xget(%0, ulocal(f.get-stat-location-on-player, special armor))), X, %b), %] Special), 1 18 *r *r *r, 0, |, %1), if(t(%2), formattext(%b, 0, %1)))
...
&layout.cg-commands [v(d.cgf)]=strcat(divider(Commands, %1), %r, multicol(+stat/set <stat>=<value>|+stat/set <stat>= to clear a stat|+stat/find <text>*|+stat/clear to start over|+stat/find <stat>=<text>|%0, * *, 0, |, %1))
