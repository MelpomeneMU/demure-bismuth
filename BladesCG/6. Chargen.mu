...
&layout.bad-or-restricted-values [v(d.cgf)]=strcat('%1' is not a value for %0. Valid values are:, %b, ulocal(layout.list, ulocal(f.list-valid-values, %0, %2))., if(t(setr(R, ulocal(layout.list, ulocal(f.list-restricted-values, %0)))), %bRestricted values are: %qR.))
