...
&tr.set-stat [v(d.cg)]=@assert t(setr(S, finditem(ulocal(f.get-stats, %2), %0, |)))={ @trigger me/tr.choose-section=%0, %1, %2;  }; @assert cand(t(strlen(setr(V, ulocal(f.get-valid-value, %qS, %1, %2)))), not(member(ulocal(f.list-restricted-values, %qS), %qV, |)))={ @trigger me/tr.error=%2, '%1' is not a value for %qS. Valid values are: [itemize(ulocal(f.list-valid-values, %qS, %2), |)].[if(t(setr(R, itemize(ulocal(f.list-restricted-values, %qS), |))), %bRestricted values are: %qR.)]; }; @assert cor(not(isapproved(%2)), member(xget(%vD, d.stats_editable_after_chargen), %qS, |))={ @assert not(isnum(%qV))={ @force %2={ +xp/buy %qS; }; }; @trigger me/tr.error=%2, %qS cannot be changed after you are approved. You will need to either %ch+xp/buy%cn or open a job with staff.; }; @assert if(ulocal(f.is-action, %qS), strcat(setq(T, ulocal(f.get-total-player-actions, %2, %qS)), lte(add(%qT, %qV), 7)), 1)={ @trigger me/tr.error=%2, Setting your %qS to %qV would take you over 7 points of actions. Reduce your action total to move the dots around.; }; @set %2=[ulocal(f.get-stat-location-on-player, %qS)]:%qV; @trigger me/tr.success=%2, You set your %ch%qS%cn to %ch%qV%cn.;

&tr.choose-section [v(d.cg)]=@assert t(setr(S, finditem(ulocal(f.get-choosable-stats, %2), %0, |)))={ @trigger me/tr.error=%2, Could not find a choosable stat that starts with '%0'.; }; @assert cand(t(strlen(setr(C, ulocal(f.get-valid-value, %qS, %1, %2)))), not(member(ulocal(f.list-restricted-values, %qS), %qC, |)))={ @trigger me/tr.error=%2, '%1' is not a value for %qS. Valid values are: [itemize(ulocal(f.list-valid-values, %qS, %2), |)].[if(t(setr(R, itemize(ulocal(f.list-restricted-values, %qS), |))), %bRestricted values are: %qR.)]; }; @assert cor(not(isapproved(%2)), member(xget(%vD, d.stats_editable_after_chargen), %qS, |))={ @trigger me/tr.error=%2, %qS cannot be changed after you are approved. You will need to open a job with staff.; }; @assert t(setr(V, switch(%qS, Rival, %qC, Ally, %qC, Special Ability, finditem(%qC, %1, |), xget(%vD, strcat(d., ulocal(f.get-stat-location, %qS.%qC))))))={ @trigger me/tr.error=%2, %qS is not set up for %qC.; }; @set %2=[ulocal(f.get-stat-location-on-player, %qS)]:%qV; @assert not(t(member(Rival|Ally|Special Ability, %qS, |)))={ @trigger me/tr.success=%2, You set your %ch%qS%cn to %ch%qV%cn.; };  @trigger me/tr.success=%2, You set your %ch%qS%cn to the %ch%qC%cn list: %ch[itemize(%qV, |, and, ;)]%cn.;
