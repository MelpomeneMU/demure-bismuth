...
&layout.xp_comments [v(d.cgf)]=squish(trim(strcat(if(t(%1), strcat(* You have %1 Untracked XP which needs to be moved onto a track.%r, space(3), %ch+xp/track <track>=<amount>%cn to move it to a useful place.)), if(%2, iter(setdiff(xget(%vG, d.xp_tracks), Untracked|Crew, |, |), if(t(setr(V, ulocal(f.get-advancements, %0, itext(0), unspent))), cat(%r*, You have %qV unspent, plural(%qV, Advancement, Advancements), in your, itext(0) track.)), |, @@))), b, %r), %r)
...
&layout.xp [v(d.cgf)]=strcat(header(XP and Advancements, %1), %r, if(setr(S, not(ulocal(f.is_expert, %0))), strcat(multicol(strcat(|, ansi(first(themecolors())u, Total XP), |, ansi(first(themecolors())u, Current/Max XP), |, ansi(first(themecolors())u, Advancements Spent/\Total), ulocal(layout.xp_track, %0, Insight), ulocal(layout.xp_track, %0, Prowess), ulocal(layout.xp_track, %0, 	Resolve), ulocal(layout.xp_track, %0, Playbook), |, ansi(first(themecolors()), Untracked), |, setr(U, ulocal(f.get-xp, %0, Untracked, total)), setq(T, ulocal(layout.xp_comments, %0, %qU, %qS)), if(t(%qT), |||)), * 12 * 25, 0, |, %1), if(t(%qT), strcat(%r, formattext(%qT%r, 0, %1))), %r)), setq(C, ulocal(f.get-player-stat, %0, crew object)), setq(T, ulocal(layout.crew-xp_comments, %qC)), multicol(strcat(|, ansi(first(themecolors())u, Total XP), |, ansi(first(themecolors())u, Current/Max XP), |, ansi(first(themecolors())u, Advancements Spent/\Total), ulocal(layout.xp_track, %qC, Crew)), * 12 * 25, 0, |, %1), if(t(%qT), strcat(formattext(%qT, 0, %1))), setq(L, ulocal(f.get-last-X-logs, %0, _xp-, 5)), if(t(%qL), strcat(%r, divider(Last 5 XP logs, %1), %r, formattext(iter(%qL, ulocal(layout.log, xget(%0, itext(0))),, %r), 0, %1))), setq(L, ulocal(f.get-last-X-logs, %0, advancement, 5)), if(t(%qL), strcat(%r, divider(Last 5 Advancement logs %(+adv for more%), %1), %r, formattext(iter(%qL, ulocal(layout.log, xget(%0, itext(0))),, %r), 0, %1))), setq(L, ulocal(f.get-last-X-logs, %0, crew_advancement, 5)), if(t(%qL), strcat(%r, divider(Last 5 Crew Advancement logs %(+adv/crew for more%), %1), %r, formattext(iter(%qL, ulocal(layout.log, xget(%0, itext(0))),, %r), 0, %1))), %r, footer(cat(ulocal(f.get-total-advancements, %0, spent), Advancements spent out of, ulocal(f.get-total-advancements, %0, total) total.), %1))
...
@@ %2: player
@@ %3: actor
&tr.xp-track [v(d.cg)]=@assert t(setr(U, ulocal(f.get-xp, %2, untracked, total)))={ @trigger me/tr.error=%3, Could not find any untracked XP to track.; }; @assert cand(isint(%1), gt(%1, 0), lte(%1, %qU))={ @trigger me/tr.error=%3, '%1' must be a number greater than 0 and less than %qU.; }; @assert t(setr(T, finditem(setr(L, setdiff(xget(%vG, d.xp_tracks), Untracked|Crew, |, |)), %0, |)))={ @trigger me/tr.error=%3, Could not find an XP track starting with '%0' to move this XP to. Valid tracks are [ulocal(layout.list, %qL)].; }; @set %2=ulocal(f.get-stat-location-on-player, xp.untracked.total):[sub(%qU, %1)]; @trigger me/tr.increase-track=%qT, %1, %2, %3, move;
...
@@ %2: advancement note
@@ %3: logging player
&tr.log-advancement [v(d.cg)]=@trigger me/tr.log=%0, strcat(switch(%1, c*, crew_), advancement_), %3, %2;

@@ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ @@
@@ +adv/log
@@ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ @@

&layout.stat-advancement [v(d.cgf)]=if(t(%2), cat(Purchased, ulocal(f.get-singular-stat-name, %2), ulocal(f.get-pretty-value, %2, %1, %3, %4).), Spent %1.)

&layout.advancement [v(d.cgf)]=if(isdbref(%1), %0, ulocal(layout.stat-advancement, %0, %1, %2, %3, %4))

&layout.advancement_log [v(d.cgf)]=strcat(header(cat(Advancement log, -, ulocal(f.get-name, %0)), %1), %r, formattext(strcat(setq(A, iter(ulocal(f.get-last-X-logs, %0, advancement, 100), ulocal(layout.log, xget(%0, itext(0))),, |)), if(t(%qA), %qA, No advancements yet.)), 0, %1), %r, footer(ulocal(layout.footer, %0, %1), %1))

&layout.crew_advancement_log [v(d.cgf)]=strcat(header(cat(Crew advancement log, -, ulocal(f.get-crew-name, %0)), %1), %r, formattext(strcat(setq(A, iter(ulocal(f.get-last-X-logs, %0, crew_advancement, 100), ulocal(layout.log, xget(%0, itext(0))),, |)), if(t(%qA), %qA, No crew advancements yet.)), 0, %1), %r, footer(ulocal(layout.footer, %0, %1), %1))
...
&c.+adv/log_add [v(d.cg)]=$+adv/log *=*:@break strmatch(%0, */*); @assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to add to a player's Advancements log.; }; @assert t(setr(P, ulocal(f.find-player, %0, %#)))={ @trigger me/tr.error=%#, Could not find a player named '%0'.; }; @trigger me/tr.log-advancement=%qP, player, ulocal(layout.advancement, %1, %#), %#; @trigger me/tr.success=%#, cat(You logged an advancement note on, ulocal(f.get-name, %qP, %#), stating: %1);
