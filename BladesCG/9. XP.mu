...
@@ %2: player
@@ %3: actor
&tr.xp-track [v(d.cg)]=@assert t(setr(U, ulocal(f.get-xp, %2, untracked, total)))={ @trigger me/tr.error=%3, Could not find any untracked XP to track.; }; @assert cand(isint(%1), gt(%1, 0), lte(%1, %qU))={ @trigger me/tr.error=%3, '%1' must be a number greater than 0 and less than %qU.; }; @assert t(setr(T, finditem(setr(L, setdiff(xget(%vD, d.xp_tracks), Untracked|Crew, |, |)), %0, |)))={ @trigger me/tr.error=%3, Could not find an XP track starting with '%0' to move this XP to. Valid tracks are [ulocal(layout.list, %qL)].; }; @set %2=ulocal(f.get-stat-location-on-player, xp.untracked.total):[sub(%qU, %1)]; @trigger me/tr.increase-track=%qT, %1, %2, %3, move;
