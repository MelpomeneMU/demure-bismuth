...
&help [search(ETHING=t(member(name(##), BUILD)))]=formattext(This bucket is for new buildings%, getting rid of buildings%, correcting typos%, etc. If it has to do with the grid%, your job belongs here.%R%R%TSee 'news build' for building information., 1, %#)

&logfile [search(ETHING=t(member(name(##), BUILD)))]=build
...
&hook_apr [search(ETHING=t(member(name(##), CG)))]=@set [xget(%0, opened_by)]=_chargen-job:; @trigger %vC/tr.approve-player=xget(%0, opened_by), %1; @trigger [v(VA)]/TRIG_LOG=%0,[v(VA)]; @trigger me/TRIG_POINTS=%1;
...
&help [search(ETHING=t(member(name(##), CG)))]=formattext(For brand new character builds only. If you have a question%, type %chcg/on%cn to join the Chargen channel., 1, %#)

&logfile [search(ETHING=t(member(name(##), CG)))]=cg
...
&hook_apr [search(ETHING=t(member(name(##), CREW)))]=@dolist [xget(%0, opened_by)]={ @set ##=_crewgen-job:; }; @trigger %vC/tr.approve-crew=xget(%0, opened_by), %1; @trigger [v(VA)]/TRIG_LOG=%0,[v(VA)]; @trigger me/TRIG_POINTS=%1;
...
&MLETTER_ADD [search(ETHING=t(member(name(##), CREW)))]=if(not(isstaff(%2)), %bComments have been added to '[name(%0)]' by [name(%2)]:%R%R %3%R%R[divider(Help, %2)]%r See '[ansi(h, +help myjobs)]' for help on how to display and add to your jobs.)
...
&help [search(ETHING=t(member(name(##), CREW)))]=formattext(For crews requesting approval only. Other types of crew-related jobs probably belong in the %chreq/char%cn%, %chreq/theme%cn%, or %chreq/social%cn., 1, %#)

&logfile [search(ETHING=t(member(name(##), CREW)))]=crew
...
&MLETTER_OTH [search(ETHING=t(member(name(##), DOWNTIME)))]=You have requested '[name(%0)]: [get(%0/TITLE)]' from staff: %r%r %3%r%r[divider(Help, %2)]%r See '[ansi(h, +help myjobs)]' for help on how to display and add to your jobs.%r You may be asked to roll to this job. If so, use %ch+roll/job <job number>=<roll>%cn.%r[divider(Turnaround, %2)]%r Some of the jobs in this bucket are meant to be kept open for weeks, so we don't list a turnaround time. For smaller jobs, you should hear from us within 3 days.

&help [search(ETHING=t(member(name(##), DOWNTIME)))]=formattext(All jobs relating to downtime%, like Long Term Projects or Acquire an Asset. If you want to spend a bunch of downtime at once%, feel free to put it in a single job even if it's related to multiple things%, but that will necessarily make the job more complex and thus it'll take longer., 1, %#)

&logfile [search(ETHING=t(member(name(##), DOWNTIME)))]=downtime
...
&MLETTER_OTH [search(ETHING=t(member(name(##), PLOTS)))]=You have requested '[name(%0)]: [get(%0/TITLE)]' from staff: %r%r %3%r%r[divider(Help, %2)]%r See '[ansi(h, +help myjobs)]' for help on how to display and add to your jobs.%r%r Please give staff at least [div(u(me/TURNAROUND), 24)] days from the date of this mail to process your request.

&help [search(ETHING=t(member(name(##), PLOTS)))]=formattext(All plot-related jobs go here. For the most part%, these will be back-and-forth jobs between players and staff about new plots the players wish to run%, but there may be other types of plot-related jobs that belong here.%R%R%TPlot jobs are expected to stay open until the plot is completed so that all the information will be in one place. Rewards related to plots belong in the plot job.%R%R%TNote that staff on this game does not run plots - we exist to facilitate player-run plots. See %chnews plots%cn for information., 1, %#)

&logfile [search(ETHING=t(member(name(##), PLOTS)))]=plots
...
&help [search(ETHING=t(member(name(##), PUBLIC)))]=formattext(This bucket is for PUBLIC jobs. Every player can see - and add to! - every job in this bucket. Comments on those jobs will be hidden unless staff runs +job/publish <job> or +job/publish <job>=<comment>.%R%R%TWe wanted a way to run polls without spamming the BBSes., 1, %#)

&logfile [search(ETHING=t(member(name(##), PUBLIC)))]=public

@@ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ @@
@@ QUERY bucket
@@ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ @@

&help [search(ETHING=t(member(name(##), QUERY)))]=formattext(When staff reaches out to a player or group of players%, the job ends up here. It can be moved from this bucket to something more relevant with +job/move <job>=<bucket>., 1, %#)

&logfile [search(ETHING=t(member(name(##), QUERY)))]=query
...
&logfile [search(ETHING=t(member(name(##), REQUESTS)))]=requests

&MLETTER_OTH [search(ETHING=t(member(name(##), REQUESTS)))]=You have requested '[name(%0)]: [get(%0/TITLE)]' from staff: %r%r %3%r%r[divider(Help, %2)]%r See '[ansi(h, +help myjobs)]' for help on how to display and add to your jobs.%r%r Please give staff at least [div(u(me/TURNAROUND), 24)] days from the date of this mail to process your request.

&help [search(ETHING=t(member(name(##), REQUESTS)))]=formattext(When no other bucket seems to fit%, put your job in this one. It can be moved with +job/move <job>=<bucket> if staff chooses., 1, %#)
...
&MLETTER_OTH [search(ETHING=t(member(name(##), SOCIAL)))]=You have alerted staff to a social issue regarding '[get(%0/TITLE)]': %r%r %3%r%r[divider(Help, %2)]%r See '[ansi(h, +help myjobs)]' for help on how to display and add to your jobs.%r[divider(Turnaround, %2)]%r Staff will be handling this ASAP. We welcome emails at [xget(%vZ, d.staff-email-address)] if you need to disconnect from the game.

&help [search(ETHING=t(member(name(##), SOCIAL)))]=formattext(Social issues belong here. These include reports of harassment%, requests for staff to help negotiate between players%, do-not-contact requests%, issues of crew membership%, etc.%R%R%T\See %chnews harassment%cn for our harassment policy.%R%R%TWe welcome emails at [xget(%vZ, d.staff-email-address)]., 1, %#)

&logfile [search(ETHING=t(member(name(##), SOCIAL)))]=social

@@ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ @@
@@ THEME bucket
@@ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ @@

&access [search(ETHING=t(member(name(##), THEME)))]=[u(%va/FN_STAFFALL, %#)]

&public [search(ETHING=t(member(name(##), THEME)))]=1

&help [search(ETHING=t(member(name(##), THEME)))]=formattext(Questions about theme? Questions about rules %(which%, given that this is a story-first game%, are also theme%)? Ask them here.%R%R%TYou may also want to try on the Rules channel with %chrul/on%cn first%, so you can get the answer faster.%R%R%TYou should also see %chnews setting%cn in case your question has already been answered., 1, %#)

&logfile [search(ETHING=t(member(name(##), THEME)))]=theme

@@ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ @@
@@ WIKI bucket
@@ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ @@

&access [search(ETHING=t(member(name(##), WIKI)))]=[u(%va/FN_STAFFALL, %#)]

&public [search(ETHING=t(member(name(##), WIKI)))]=1

&turnaround [search(ETHING=t(member(name(##), WIKI)))]=24

&help [search(ETHING=t(member(name(##), WIKI)))]=formattext(If you want a wiki login%, send us your email and your preferred login name.%R%R%TIf you want new features on the wiki%, that's a harder ask and less likely to succeed. We wrench on the wiki maybe once a year and always with trepidation.%R%R%TIf you find a bug or problem with the wiki%, report it here! We can't promise we'll be able to fix it%, but we'll at least look at it disapprovingly and maybe try a few basic troubleshooting steps., 1, %#)

&logfile [search(ETHING=t(member(name(##), WIKI)))]=wiki
